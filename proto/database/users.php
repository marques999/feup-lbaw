<?
	...
	function users_listById($user_id) {
		global $db;
		$stmt = $db->prepare('SELECT * FROM Utilizador WHERE idUser = :idUser');
	...
	function users_getNextId() {
		...
		global $db;
		$stmt = $db->prepare("SELECT * FROM SQLITE_SEQUENCE WHERE name='Utilizador'");
		$stmt->execute();
		$result = $stmt->fetch();
	...
	function numberUtilizador() {

		global $db;
		$stmt = $db->prepare('SELECT COUNT(*) AS count FROM Utilizador');
		$stmt->execute();
	...
	function users_countInvites($user_id) {
		...
		$stmt = $db->prepare('SELECT COUNT(Invites.idEvent) AS count FROM Invites
			...
			AND Utilizador.idUser = Invites.idUser');
		$stmt->bindParam(':idUser', $user_id, PDO::PARAM_INT);
		$stmt->execute();
		$result = $stmt->fetchAll();
